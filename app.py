...
def inject_tactical_styles():
    st.markdown("""
        ...
        .stApp {background-color: #0b0d11; color: #aeb9cc; font-family: 'Roboto Mono', monospace; font-size: 11px;}
        ...
        .stButton > button {background-color: #1f6feb; color: white; border: none; border-radius: 0px; padding: 10px 20px; font-size: 11px; text-transform: uppercase; font-weight: bold; width: 100%;}
        .stButton > button:hover {background-color: #388bfd;}
        .card {background: #161b22; padding: 1.5rem; border-radius: 8px; border: 1px solid #30363d; margin-bottom: 1.5rem; position: relative;}
        .close-btn {position: absolute; top: 8px; right: 12px; background: none !important; border: none !important; font-size: 20px !important; color: #8b949e !important; cursor: pointer; padding: 0 !important; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; border-radius: 50%; transition: all 0.2s;}
        .close-btn:hover {color: #f85149 !important; background-color: rgba(248, 81, 73, 0.1) !important;}
        .footer {position: fixed; left: 0; bottom: 0; width: 100%; background-color: #010409; color: #484f58; text-align: right; padding: 8px 20px; font-size: 9px; border-top: 1px solid #30363d; z-index: 999;}
        </style>
...
MODEL_IDS = {
    "ResNet-101": "1AyOGHg3lxc0f1a2v-tcePPVg8L7wwmXR",
    "ConvNeXt Base": "1pCWUKGl8-u0wP6tPbKFYbxH2FrHBqZvR",
    "CNN Scratch": "17XATLBwfsH1BacU8AuQoZ13c4I_-WrHX",
}

ASSET_PATHS = {
    "ResNet-101": {
        "curve": "https://drive.google.com/uc?export=download&id=1xA5BQQZNd1bO0mxvbwiMA-wCj_c8SMlK",
        "matrix": "https://drive.google.com/uc?export=download&id=1KlEXH50htRWRyz5PZCyFV1xzjRmXPq6h"
    },
    "ConvNeXt Base": {
        "curve": "https://drive.google.com/uc?export=download&id=1sNQg3wEqMSFoWqjOKrNaPlj6TF-5fWx3",
        "matrix": "https://drive.google.com/uc?export=download&id=1OYf1yXp6F1FXCewwKoFS9H_vZdsJDPVk"
    },
    "CNN Scratch": {
        "curve": "https://drive.google.com/uc?export=download&id=1MMeRLV6z3fFNSvncexsWMRUaQncaRzCT",
        "matrix": "https://drive.google.com/uc?export=download&id=1L9bU6Wqo7RSHG7JZufvXa_RAt7dGQtPy"
    }
}
...
class CustomCNN(nn.Module):
    def __init__(self, num_classes=22):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(64, 128, 3, padding=1), nn.BatchNorm2d(128), nn.ReLU(inplace=True),  # match checkpoint
...
def remove_backbone_prefix(sd):
    new_sd = {}
    for k, v in sd.items():
        new_sd[k.replace("backbone.", "") if k.startswith("backbone.") else k] = v
    return new_sd

@st.cache_resource(show_spinner=False)
def load_model(name: str):
    file_id = MODEL_IDS[name]
    filename = f"{name.replace('-', '').replace(' ', '').lower()}.pkl"
    ...
    sd = remove_backbone_prefix(data["model_state_dict"])
    ...
    if name == "ResNet-101":
        ...
        model = create_model("convnext_base", pretrained=False, num_classes=1000)
        model.load_state_dict(sd, strict=False)
        model.head.fc = nn.Linear(model.head.fc.in_features, 22)
    elif name == "CNN Scratch":
        model = CustomCNN(num_classes=22)
        # Load only features to avoid mismatch
        feature_sd = {k: v for k, v in sd.items() if k.startswith('features.')}
        model.features.load_state_dict(feature_sd, strict=False)
        # Reinitialize classifier (random weights)
...
for name in MODEL_IDS:
    try:
        ...
        st.warning(f"Failed loading {name}: {e}")
...
if st.session_state.get("show_dashboard", False):
    ...
    for model_name in available_models:
        ...
        with col1:
            try:
                response = requests.get(assets["curve"])
                st.image(response.content, caption="Training & Validation Curves", use_container_width=True)
            except:
                st.error("Failed to load curve image")
        
        with col2:
            try:
                response = requests.get(assets["matrix"])
                st.image(response.content, caption="Confusion Matrix", use_container_width=True)
            except:
                st.error("Failed to load matrix image")
...
def generate_heatmap(model, img_tensor):
    if "CustomCNN" in str(type(model).__name__):
        target_layer = None
...
def overlay_heatmap(img_np, cam):
    ...
    heatmap = cv2.applyColorMap(np.uint8(255 * cam_resized), cv2.COLORMAP_JET) # pyright: ignore[reportArgumentType, reportCallIssue] # pyright: ignore[reportCallIssue]
    ...
    if analyze_btn or 'result' in st.session_state:
        if analyze_btn:
            ...
            with st.spinner(f"ANALYZING WITH {method.upper()}..."):
                model = models[method]
                x = preprocess(image_pil).unsqueeze(0).to(DEVICE) # pyright: ignore[reportAttributeAccessIssue]
